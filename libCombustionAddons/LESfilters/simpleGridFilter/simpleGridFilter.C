...
) const
{
  ...
  //filteredField = filteredField / fvc::surfaceSum(mesh().magSf());
...
) const
{
  ...
  forAll(owner, faceI)
    {
      //if (owner[faceI]==100) printf("X %d\n", faceI);
      filteredField[owner[faceI]]+=
	0.5*( surfvals[faceI] + uf[owner[faceI]] ) * mesh().magSf()[faceI];
      
      //if (neighbour[faceI]==100) printf("Z %d\n", faceI);
      filteredField[neighbour[faceI]]+=
	0.5*( surfvals[faceI] + uf[neighbour[faceI]] ) * mesh().magSf()[faceI];
  ...
  forAll(mesh().boundary(), patchI)
	...
	mesh().boundary()[patchI].faceCells();
      ...
      forAll(mesh().boundary()[patchI], faceI)
        {
	  //if (pFaceCells[faceI]==100) printf("Y\n");
	  //Info<<"Facecell "<<pFaceCells[faceI]<<endl;
	  //Info<<"FaceI="<<faceI<<endl;
	  filteredField[pFaceCells[faceI]]+=
	    0.5*(ufsf[faceI]+uf[pFaceCells[faceI]]) * mesh().boundary()[patchI].magSf()[faceI];
	}
    }
